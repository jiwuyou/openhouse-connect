(() => {
  ...
  const isImageAttachment = (att) => {
    ...
    const type = (att && (att.content_type || att.mime || att.type)) || "";
  ...
  const formatTime = (m) => {
    const raw = m.created_at ?? m.createdAt ?? m.ts ?? m.time ?? null;
    if (!raw) return "";
  ...
  const renderMessage = (m) => {
    ...
    if (Array.isArray(atts) && atts.length) {
      ...
      for (const a of atts) {
        ...
        const tag = (a && (a.content_type || a.mime || a.type || a.kind)) || "";
  ...
  const startEvents = async () => {
    ...
    es.onopen = async () => {
      ...
      reconnectAttempt = 0;
      setStatus("ok", "connected");
      // Resync only on reconnect to avoid double-loading on initial connect.
      if (isReconnect) {
        try {
          await loadHistory();
        } catch {}
  ...
  const init = async () => {
    ...
    refreshSessionsBtn.addEventListener("click", async () => {
      try {
        ...
        // No modal; status pill is enough.
      }
    });
    ...
    composerForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        autosizeTextarea();
        composerInput.focus();
        // Expect SSE to deliver echoed message; resync defensively.
        await sleep(120);
        await loadHistory();
      } catch (err) {
