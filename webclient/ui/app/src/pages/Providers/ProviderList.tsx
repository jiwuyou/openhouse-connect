  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        ...
        <Button className="max-sm:w-full" onClick={() => { setEditProvider(null); setShowAddModal(true); }}>
      ...
      <div className="flex gap-1 p-1 rounded-xl bg-gray-100 dark:bg-white/[0.06] w-full sm:w-fit">
        {(['providers', 'presets'] as const).map(key => (
          <button
            ...
            onClick={() => setTab(key)}
            className={cn(
              'flex-1 sm:flex-none px-4 py-2 sm:py-1.5 rounded-lg text-sm font-medium transition-all',
              tab === key
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title={t('common.confirmDelete')}>
        ...
        <div className="flex justify-end gap-2 flex-wrap">
          <Button variant="ghost" className="max-sm:flex-1" onClick={() => setDeleteTarget(null)}>{t('common.cancel')}</Button>
          <Button variant="danger" className="max-sm:flex-1" onClick={handleDelete}>{t('common.delete')}</Button>
        </div>
      </Modal>
    </div>
  );
        ...
        <Card key={p.name} className="group relative">
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
              <button
                onClick={() => onEdit(p)}
                className="p-2 min-h-10 min-w-10 rounded-lg hover:bg-gray-100 dark:hover:bg-white/[0.06] text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"
              >
                <Pencil size={14} />
              </button>
              <button
                onClick={() => onDelete(p.name)}
                className="p-2 min-h-10 min-w-10 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10 text-gray-400 hover:text-red-500"
              >
                <Trash2 size={14} />
              </button>
            </div>
    ...
    <Modal open onClose={onClose} title={isEdit ? t('globalProviders.edit') : t('globalProviders.add')}>
      <div className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 p-2 min-h-10 min-w-10 text-gray-400 hover:text-gray-600"
              >
        ...
        <div className="flex justify-end gap-2 pt-2 flex-wrap">
          <Button variant="ghost" className="max-sm:flex-1" onClick={onClose}>{t('common.cancel')}</Button>
          <Button className="max-sm:flex-1" onClick={handleSubmit} disabled={!form.name || saving}>
