...
export default function CommandPalette({ open, onClose, onSelect, anchorRef }: Props) {
  ...
  return (
    <div
      ref={panelRef}
      className={cn(
        'fixed inset-x-2 bottom-2 max-h-[70dvh] flex flex-col rounded-xl overflow-hidden z-50 safe-bottom',
        'sm:absolute sm:inset-x-auto sm:bottom-full sm:left-0 sm:mb-2 sm:w-80 sm:max-h-[420px]',
            ...
            {g.items.map((cmd) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'w-full flex items-center gap-2.5 px-2.5 py-2.5 sm:py-2 rounded-lg text-sm transition-colors',
                    idx === activeIdx
                ...
                >
                  ...
                  <span className="font-mono text-xs text-gray-500 dark:text-gray-400 w-20 sm:w-24 text-left shrink-0">{cmd.cmd}</span>
