...
import { getAgentLabel, isProviderCompatible } from '@/lib/providers';
...
export default function ProjectDetail() {
  ...
  const [platformDeleteTarget, setPlatformDeleteTarget] = useState<{ index: number; type: string } | null>(null);
  const [deletingPlatform, setDeletingPlatform] = useState(false);
  const [platformDeleteError, setPlatformDeleteError] = useState('');
  ...
  const platformDeleteErrorMessage = (e: any) => {
    if (e?.status === 409) {
      return t('setup.platformDeleteAmbiguous', 'Multiple platforms match this selector. Refresh the project and delete the platform by row index.');
    }
    if (e?.status === 404) {
      return t('setup.platformDeleteNotFound', 'Platform or project was not found. Refresh the project and try again.');
    }
    return e?.message || String(e);
  };

  const handleDeletePlatform = async () => {
    if (!name || !platformDeleteTarget) return;
    setDeletingPlatform(true);
    setPlatformDeleteError('');
    try {
      const res = await deleteProjectPlatform(name, String(platformDeleteTarget.index));
      setPlatformDeleteTarget(null);
      await fetchAll();
      if (res.restart_required) setShowRestartModal(true);
    } catch (e: any) {
      setPlatformDeleteError(platformDeleteErrorMessage(e));
    } finally {
  ...
  const platformRows = project
    ? (project.platform_configs?.length ? project.platform_configs : project.platforms || []).map((p, index) => ({
        index,
        type: p.type,
        connected: project.platforms?.some((runtime) => runtime.type === p.type && runtime.connected) ?? false,
      }))
    : [];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <h2 className="text-xl font-bold text-gray-900 dark:text-white truncate">{project?.display_name || name}</h2>
        {project && <Badge variant="info">{getAgentLabel(project.agent_type)}</Badge>}
      </div>

      {/* Tabs */}
      <div className="flex gap-2 overflow-x-auto pb-1 mobile-scrollbar-none">
        {tabs.map(({ key, icon: Icon }) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={cn(
              'flex items-center gap-2 px-3 sm:px-4 py-2 min-h-10 rounded-lg text-sm font-medium transition-all whitespace-nowrap shrink-0',
              tab === key
        ...
        <div className="space-y-4">
          <Card>
            ...
            {platformRows.length ? (
                ...
                {platformRows.map((p) => (
                  <div
                    ...
                    className={cn(
                      ...
                      p.connected
                        ? 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-900/20 dark:text-emerald-300'
                  ...
                  >
                    ...
                    <span>{p.type}</span>
                    <span className="text-[11px] opacity-80">{p.connected ? 'connected' : 'offline'}</span>
                    <button
                      type="button"
                      onClick={() => {
                        setPlatformDeleteError('');
                        setPlatformDeleteTarget({ index: p.index, type: p.type });
                      }}
                      className="ml-1 rounded-md p-1 text-gray-400 transition-colors hover:bg-red-500/10 hover:text-red-500"
                      title={t('setup.removePlatform', 'Remove platform')}
                      aria-label={t('setup.removePlatformLabel', 'Remove {{platform}} platform', { platform: p.type })}
                    >
                      <Trash2 size={12} />
                    </button>
                  </div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{t('setup.noPlatforms', 'No platforms configured.')}</p>
            )}
          </Card>
          <Card>
      ...
      {tab === 'providers' && (() => {
        ...
        const projectAgentType = project?.agent_type || '';
        const unlinkedGlobals = globalProviders.filter(g =>
          !providerRefs.includes(g.name) && isProviderCompatible(projectAgentType, g)
        );
        return (
        <div className="space-y-4">
          {/* Header */}
            ...
            <div className="space-y-2">
              {providers.map((p) => (
                <div
                  key={p.name}
                  className={cn(
                    'flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 rounded-xl border transition-all',
                    p.active
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 shrink-0 sm:ml-3 self-end sm:self-auto flex-wrap justify-end">
                    {!p.active && (
            ...
            <div className="space-y-4">
              ...
              {addMode === 'pick' ? (
                unlinkedGlobals.length === 0 ? (
                  ...
                  <div className="space-y-1.5 max-h-[55dvh] sm:max-h-64 overflow-y-auto pr-1">
                    {unlinkedGlobals.map(gp => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          {gp.agent_types && gp.agent_types.length > 0 && (
                            <div className="mt-1 text-[11px] text-gray-400">
                              {gp.agent_types.map(getAgentLabel).join(', ')}
                            </div>
                          )}
                        </div>
          ...
          <Modal open={showInterval} onClose={() => setShowInterval(false)} title={t('heartbeat.setInterval')}>
            <div className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2 flex-wrap">
                <Button variant="secondary" className="max-sm:flex-1" onClick={() => setShowInterval(false)}>{t('common.cancel')}</Button>
                <Button className="max-sm:flex-1" onClick={handleSetInterval}>{t('common.save')}</Button>
              </div>
            </div>
          </Modal>
        </div>
      ...
      {tab === 'settings' && project && (
          ...
          <div className="space-y-4 max-w-lg">
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-gray-400 mt-0.5">{t('projects.replyFooterHint', 'Show model, effort, quota, and workdir footer on replies')}</p>
              </div>
              <button
        ...
        <div className="max-w-lg">
          <Button className="max-sm:w-full" loading={saving} onClick={handleSaveSettings}>{t('common.save')}</Button>
        </div>
        <Card>
      ...
      <Modal open={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title={t('projects.deleteTitle')}>
        <div className="space-y-4 py-2">
          ...
          <div className="flex justify-end gap-2 flex-wrap">
            <Button variant="secondary" className="max-sm:flex-1" onClick={() => setShowDeleteConfirm(false)}>{t('common.cancel')}</Button>
            <Button variant="danger" className="max-sm:flex-1" onClick={handleDeleteProject} disabled={deleting}>
      ...
      <Modal
        ...
        onClose={() => {
          if (!deletingPlatform) setPlatformDeleteTarget(null);
        }}
        title={t('setup.removePlatform', 'Remove platform')}
      >
        <div className="space-y-4 py-2">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {t('setup.removePlatformConfirm', 'Remove {{platform}} from this project config. The running connection is not disconnected immediately; restart the service for the change to take effect.', { platform: platformDeleteTarget?.type })}
          </p>
          {platformDeleteError && (
          ...
          <div className="flex justify-end gap-2 flex-wrap">
            <Button variant="secondary" className="max-sm:flex-1" onClick={() => setPlatformDeleteTarget(null)} disabled={deletingPlatform}>
              {t('common.cancel')}
            </Button>
            <Button variant="danger" className="max-sm:flex-1" onClick={handleDeletePlatform} loading={deletingPlatform}>
              {t('common.delete')}
            </Button>
          </div>
        </div>
      ...
      <Modal open={showRestartModal} onClose={() => setShowRestartModal(false)} title={t('setup.restartRequired', 'Restart required')}>
        <div className="space-y-4 py-2">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {t('setup.restartConfigHint', 'Restart the service for platform configuration changes to take effect.')}
          </p>
          <div className="flex justify-end gap-2 flex-wrap">
            <Button variant="secondary" className="max-sm:flex-1" onClick={() => { setShowRestartModal(false); fetchAll(); }}>
              {t('setup.later', 'Later')}
            </Button>
            <Button className="max-sm:flex-1" onClick={async () => { await restartSystem(); setShowRestartModal(false); fetchAll(); }}>
