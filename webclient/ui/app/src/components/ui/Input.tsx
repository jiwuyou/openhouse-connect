...
export function Input({ label, className, ...props }: InputProps) {
  return (
    <div className="space-y-1.5">
      ...
      <input
        className={cn(
          'w-full min-h-10 px-3 py-2 text-sm rounded-lg transition-all duration-200',
...
export function Textarea({ label, className, ...props }: TextareaProps) {
  return (
    <div className="space-y-1.5">
      ...
      <textarea
        className={cn(
          'w-full min-h-24 px-3 py-2 text-sm rounded-lg transition-all duration-200 resize-none',
          'border border-gray-300/90 dark:border-white/[0.1]',
          'bg-white/90 backdrop-blur-sm dark:bg-[rgba(0,0,0,0.45)] dark:backdrop-blur-md',
          'text-gray-900 dark:text-white',
          'focus:outline-none focus:ring-2 focus:ring-accent/45 focus:border-accent',
