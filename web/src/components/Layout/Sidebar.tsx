...
const navItems = [
  ...
  { key: 'help', path: '/help', icon: BookOpen },
...
export default function Sidebar({ mobile = false, onNavigate }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        'h-dvh flex flex-col border-r transition-all duration-300 ease-out',
        'bg-white/75 backdrop-blur-xl border-gray-200/80',
        'dark:bg-[rgba(0,0,0,0.85)] dark:backdrop-blur-xl dark:border-white/[0.08]',
        mobile ? 'w-72 max-w-[calc(100vw-3rem)] shadow-2xl shadow-black/20' : isCollapsed ? 'w-16' : 'w-56',
      )}
    >
      {/* Logo */}
      <div
      ...
      {/* Collapse toggle */}
      {!mobile && <div className={cn('border-t p-2', 'border-gray-200/80 dark:border-white/[0.08]')}>
        <button
          type="button"
